					...
					<tr>
						...
						<td>
							...
							<form action="updateOfficial.php" method="post">
								<button type="submit" class="btn btn-success" name="edit"><i class="fas fa-pencil-alt"></i></button>
						...
						<td>
							<form action="deleteOfficial.php" method="post">
								...
								<input type="hidden" id="deleteIdOfficial" name="deleteIdOfficial" value="<?php echo $results['id_official']?>">
							</form>
						</td>
					</tr>
				<?php } ?>
	...
	<div class="card-body">
		<div class="table-responsive">
			<table class="table table-hover" id="dataTable" width="100%" cellspacing="0">
				...
				<?php 
						$sql = "SELECT * FROM tbluser WHERE Status = '1' AND position = 'Punong Barangay' || position = 'Secretary'";
						$res = mysqli_query($cn, $sql);
						while ($results = mysqli_fetch_array($res)) { ?>
					<tr>
						<td><?php echo $results ['user_id']?></td>
						<td><?php echo $results ['lname'], ", ", $results ['fname'], " ", $results ['mname'], " ", $results ['suffix'] ?></td>
						<td><?php echo $results ['position'] ?></td>
						...
						<td>
							...
							<form action="updateUser.php" method="post">
								<button type="submit" class="btn btn-success" name="edit"><i class="fas fa-pencil-alt"></i></button>
								<input type="hidden" name="editIdOfficial" value="<?php echo $results ['user_id'] ?>">
								<input type="hidden" name="editlname_official" value="<?php echo $results ['lname'] ?>">
								<input type="hidden" name="editfname_official" value="<?php echo $results ['fname'] ?>">
								<input type="hidden" name="editmname_official" value="<?php echo $results ['mname'] ?>">
								<input type="hidden" name="editsuffix_official" value="<?php echo $results ['suffix'] ?>">
								<input type="hidden" name="editusername" value="<?php echo $results ['user_name'] ?>">
								<input type="hidden" name="editpassword" value="<?php echo $results ['user_password'] ?>">
								<input type="hidden" name="editdob" value="<?php echo $results ['birthdate'] ?>">
								<input type="hidden" name="editcnum" value="<?php echo $results ['mobile_number'] ?>">
								<input type="hidden" name="editPosition" value="<?php echo $results ['position'] ?>">
							</form>

						</td>
						<td>
							<form action="/BarangayInfoSystem/Admin/deleteOfficial.php" method="post">
								<button type="submit" class="btn btn-danger" name="delete"><i class="fas fa-trash-alt"></i></button>
								<input type="hidden" name="deleteIdOfficial" value="<?php echo $results['user_id']?>">
							</form>
						</td>
					</tr>
				<?php } ?>
